export default {
  translations: {
    status: {
      info: 'محصّن',
      desc: 'أكمل جرعات لقاح كورونا (كوفيد-19)',
      lastUpdate: 'آخر تحديث'
    },
    home: {
      // newServices: 'الخادمات الحديثة',
      newServices: 'أبرز الخدمات',
      displayAll: 'عرض الكل'
    },
    sPage: {
      publicS: 'الخدمات العامة',
      permitS: 'خدمات التصاريح',
      healthS: 'خدمات الصحة',
      educationS: 'خدمات التعليم',
      dependentsS: 'خدمات التابعين',
      hajjS: 'خدمات الحج والعمرة',
    },
    card: {
      handheart: 'التبرع بالأعضاء',
      // INFO public
      family2: 'تعريف رقم الجوال',
      // INFO permit
      plus: 'طلب تصريح جديد',
      time: 'التصاريح الشخصية',
      people: 'تصاريح التجمع',
      // INFO health
      warning: 'الوضع الحذر',
      microscope: 'فحص كورونا',
      question: 'اسأل عن مساعدة',
      passport: 'الجواز الصحي',
      vaccine: 'لقاح كورونا',
      // INFO education
      certificate: 'الحالة الدراسية للأبناء',
      madrasa: 'بيانات منصة مدرستي',
      // INFO dependents
      cCard: 'بطاقة الوضع الصحي',
      people3: 'رعاية التابعين',
      family: 'أفراد الأسرة والمكفولين',
      // INFO Hajj
      permits: 'التصاريح',
      issuepermits: 'إصدار التصاريح',
      people2: 'إدارة المستفيدين',
      // INFO no use now
      // test: 'COVID-19 Test',
      // cirtifyNumber: 'Certify Mobile Number'
      permitsUmra: 'إصدار التصاريح',
      skoraya: 'أسعفني',
      qrcode: 'ماسح كود التحقيق'
    },
    tabaut: {
      title: 'تحذير',
      desc: 'نحثك بشدة بتحميل تطبيق تباعد..',
      text: 'الذي يشعرك في حال ثبوت مخالطتك بمصاب بفايروس كورونا في الأسبوعين الماضيين ، كما يمكنك من إشعار كل الذين خالطتهم خلال 14 يوما عند إصابتك بفيروس كورونا -لا سمح الله- دون خرق لخصوصيتك أو مشاركة بياناتك الشخصية مع الآخرين . شعورك بالمسؤولية يحمينا جميعا من تفشي الفيروس .',
    },
    menu: {
      home: 'الرئيسية',
      services: 'الخدمات',
      documents: 'المحفظة الرقمية',
      dashboard: 'لوحة البيانات',
      profile: 'حسابي'
    }
  }
}
